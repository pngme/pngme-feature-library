...
def get_institution_balances(client: Client, user_uuid: str) -> dict:
    ...
    for institution in institutions:
        ...
        balances = [
            ...
            if (balance.account_type == "depository") or (balance.account_type == "mobilemoney") 
        ]
...
def get_eod_balance(
    ...
    sod_sheet = (
        pd.concat(
            [eod_sheet, sod_sheet], axis=0, ignore_index=True
        )
        .sort_values("ts")
        .fillna(method="ffill")
    )
    ...
    eod_balance = (
        sod_sheet[sod_sheet["end_of_day"] == True][
            ["ts", "balance"]
        ]
        .set_index("ts")
...
def get_avg_daily_balance(
    ...
    for institution_id, balances in account_balances.items():
        if not balances.empty:
            eod_balance = get_eod_balance(
                institution_id, 
                balances, 
                start_ts, 
                end_ts
            )
            if eod_balance.isnull().any().any():
                print(
                    f"WARNING: There are periods of time balance for {institution_id} is unkown. Will just consider as 0 now."
                )
...
if __name__ == "__main__":
    ...
    account_balances = get_institution_balances(
        client=client, 
        user_uuid=user_uuid
    )
    ...
    avg_daily_balance = get_avg_daily_balance(
            account_balances, 
            utc_starttime=utc_starttime, 
            utc_endtime=utc_endtime
        )

    print(avg_daily_balance)